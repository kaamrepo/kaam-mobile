          ...
          items.map((item, index) => (
            <Pressable
              onPress={pressed => {
                ...
                const selectedItems = updatedItems.filter(
              ...
              style={({pressed}) => [
                tw`rounded-full my-1 mr-1 flex self-start border ${
                  item.isSelected
                    ? ' bg-[#4A9D58] border-[#4A9D58]'
                    : ' border-slate-400 '
                } px-4 py-[5px]
              ...
              <Text
                numberOfLines={1}
                style={[
                  tw` ${item.isSelected ? 'text-white' : 'text-slate-500'}`,
