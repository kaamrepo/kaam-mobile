...
//     console.log("handleIntiaatePress called +++++",data?._id);
//     navigation.navigate('EmployeeDetails', { id: data?._id,index:5 });
//   };
...
const [genderFilter, setGenderFilter] = useState(null);
        ...
        <Pressable
          ...
          <View style={tw`flex`}>
            <Text
              ...
              {user?.item?.address?.city || 'City - NA'}/
              {user?.item?.address?.state || 'State - NA'}
            </Text>
          </View>
        </Pressable>
      );
  ...
  const listFooterComponent = useCallback(()=> {
    return(
      <ActivityIndicator size={'large'} style={{marginVertical:16}}></ActivityIndicator>
    )
  },[]);
  const ItemSeparatorComponent = useCallback(item => {
    return <View style={tw`justify-center`}></View>;
  });
  const onEndReached = async item => {
  ...
  return (
    <SafeAreaView style={tw`flex-1 bg-slate-50 mt-10`} edges={['top']}>
      <View style={tw`flex-row items-center mb-4 mt-2`}>
        ...
        <Modal transparent={true} animationType="slide" style={tw`shadow-2xl`}>
          <View style={tw`flex-1 justify-center items-center shadow-2xl`}>
            <View style={tw`bg-white p-4 rounded-lg shadow-md w-80`}>
              <Text style={tw`font-bold text-lg mb-2`}>Select an Option:</Text>
              <View style={tw`flex-row items-center mb-2`}>
